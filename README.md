...
App per Android Auto che avvisa in tempo reale quando arriva una nuova mail, mostrandone mittente, oggetto e testo al tocco della notifica. Nessuna risposta possibile: solo notifica ed elenco.
...
**Attenzione ai duplicati**: se per lo stesso account attivi entrambe le modalità (es. Libero letto sia via IMAP diretto sia ancora notificato da Outlook), riceverai la notifica due volte, perché i due canali sono indipendenti e non sanno riconoscere che è la stessa mail. Se aggiungi un account in IMAP diretto, vai nell'app Outlook → Impostazioni → Notifiche → quell'account, e disattiva le notifiche lì, lasciando che sia solo l'IMAP diretto a occuparsene.

**Risparmio energetico**: in Impostazioni c'è un pulsante "Disattiva risparmio energetico per questa app". Su molti telefoni (soprattutto Xiaomi/MIUI, Huawei, Oppo/ColorOS) il sistema interrompe periodicamente i servizi in background per risparmiare batteria, impedendo alle notifiche di arrivare in modo continuo in entrambe le modalità: è consigliato disattivarlo per questa app.
...
Nella tendina Provider c'è anche **"Altro (manuale)"**: sceglilo per configurare qualsiasi altro account IMAP (non solo Gmail/Libero/Aruba) inserendo tu stesso server, porta e sicurezza — i tre campi si sbloccano per la scrittura solo con questa opzione, con gli altri provider restano precompilati e bloccati.
